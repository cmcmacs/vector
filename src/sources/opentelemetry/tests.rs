...
use vector_lib::opentelemetry::proto::metrics::v1::{AggregationTemporality, ExponentialHistogram, ExponentialHistogramDataPoint, Gauge, Histogram, HistogramDataPoint, Metric, NumberDataPoint, ResourceMetrics, Summary, ScopeMetrics, Sum, SummaryDataPoint};
...
use crate::{
    ...
    event::{into_event_stream, Event, EventStatus, LogEvent, Metric as MetricEvent, ObjectMap, Value},
    ...
    assert_source_compliance(&SOURCE_TAGS, async {
        ...
        let mut expected_event = Event::from(MetricEvent::new(
            ...
            MetricValue::AggregatedHistogram {
                buckets: vec![
                    Bucket { count: 1, upper_limit: 50.0 },
                    Bucket { count: 2, upper_limit: 100.0 },
                    Bucket { count: 2, upper_limit: 150.0 },
                    Bucket { count: 4, upper_limit: f64::INFINITY },
                ],
                count: 9,
                sum: 123.45,
            },
...
async fn receive_expontential_histogram_metric() {
    assert_source_compliance(&SOURCE_TAGS, async {
        ...
        let mut expected_event = Event::from(MetricEvent::new(
            ...
            MetricValue::AggregatedHistogram {
                buckets: vec![
                    Bucket { count: 1, upper_limit: -0.8408964152537146 },
                    Bucket { count: 2, upper_limit: -1.0 },
                    Bucket { count: 1, upper_limit: 0f64 },
                    Bucket { count: 2, upper_limit: 1.189207115002721 },
                    Bucket { count: 1, upper_limit: 1.4142135623730951 },
                ],
                count: 7,
                sum: 700.00,
            },
    ...
    assert_source_compliance(&SOURCE_TAGS, async {
        ...
        let mut expected_event = Event::from(MetricEvent::new(
            ...
            MetricValue::AggregatedSummary {
                quantiles: vec![
                    Quantile{quantile: 0.5, value: 24.5},
                    Quantile{quantile: 0.9, value: 45.0},
                    Quantile{quantile: 1.0, value: 60.0},
                ],
                count: 5,
                sum: 122.5,
            },
