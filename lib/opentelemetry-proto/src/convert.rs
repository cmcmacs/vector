        ...
        self.scope_metrics
            ...
            .flat_map(move |scope_metrics| {
                ...
                scope_metrics.metrics.into_iter().flat_map(move |metric| {
                    ...
                    match metric.data {
                        Some(Data::Gauge(g)) => {
                            Self::convert_to_gauge(g, &resource, &scope, &metric_name)
                        }
                        Some(Data::Sum(s)) => {
                            Self::convert_to_sum(s, &resource, &scope, &metric_name)
                        }
                        Some(Data::Histogram(h)) => {
                            Self::convert_to_histogram(h, &resource, &scope, &metric_name)
                        }
                        Some(Data::ExponentialHistogram(e)) => {
                            Self::convert_to_exp_histogram(e, &resource, &scope, &metric_name)
                        }
                        Some(Data::Summary(su)) => {
                            Self::convert_to_summary(su, &resource, &scope, &metric_name)
                        }
                        _ => Vec::new(),
                    }
                })
